...
def bilstm_cross_attn_focal_model():
    ...
    bi_lstm = Bidirectional(LSTM(256, kernel_regularizer=regularizers.l1_l2(l1=0.00001, l2=0.00001), 
                                ...
                                recurrent_constraint=None, bias_constraint=None, dropout=0.2, return_sequences=True), 
                                input_shape=(1000, 83), name = 'BI_LSTM')(inputs) 

    att, att_weight = attention()(bi_lstm)
    ...
    model.compile(loss=focal_loss(gamma=5., alpha=0.5), optimizer='adam', metrics=['accuracy'])

    es = EarlyStopping(monitor='val_loss', mode='min', verbose=5, patience=20)
    mc = ModelCheckpoint('../../bilstm_focal_model_wts.h5', monitor='val_loss', mode='min', save_best_only=True, save_weights = True, verbose=1)
    ...
    print()
    print()
    print('TESTING for BI_LSTM ATTENTION with FOCAL_LOSS.....................................')
    print()
    model.load_weights('../../bilstm_focal_model_wts.h5')
