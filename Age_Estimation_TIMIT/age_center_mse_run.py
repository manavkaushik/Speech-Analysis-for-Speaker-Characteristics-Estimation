...
if __name__ == '__main__':   
    ...
    # 1. LOAD ENVIRONMENT
    ################ Loading GPU or CPU ###########################################################################
    device = modules_age.pytorch_env()
    ...
    params = dict(
        ...
        criterion_age = nn.MSELoss(),                        # Losses of BackProp
        criterion_center = modules_age.CenterLoss(num_classes=13, feat_dim=864, use_gpu=True),     # Losses of BackProp: Center Loss
        ...
        output_size = 1,                                    # Number of Outputs (1 for age estimation)
    ...
    csv_logger = CSVLogger('./', name='age_center_mse', version='0') # Creates a CSV in the folder which contains all the logs (Training + Testing + Validation)
    ...
    dm = modules_age.Data_Module_age_center(
    ...
    model = modules_age.lstm_crossattn_center(
                    n_features = params['n_features'],
                    hidden_size = params['hidden_size'],
                    seq_len = params['seq_len'],
                    batch_size = params['batch_size'],
                    criterion_age = params['criterion_age'], criterion_center = params['criterion_center'],
    ...
    print()
    print('##################### TESTING for AGE ESTIMATION on Test_Set #####################################')
    ...
    print(f"RMSE Age on all Female data: {np.sqrt(err_mse_female.cpu().numpy())}")
    print(f"MAE Age on all Female data: {err_mae_female}")
    print(f"RMSE Age on all Male data: {np.sqrt(err_mse_male.cpu().numpy())}")
    print(f"MAE Age on all Male data: {err_mae_male}")
    print()